...
case class initNodes(nodeId: String, nodeArray: Array[Array[Array[ActorRef]]], rumorLimit: Int, meshSize: Int, master: ActorRef, s: Double, w: Double, topology: String)
case object SendGossip
case object TickerGossip
case object TickerPushSum
case class TimeKeeper3d(nodeStopped: Boolean)
case class initBuildTime(startTime: Long, totalNodes: Int)
...
class timeCalculator extends Actor {
  ...
  def receive = {
    case initBuildTime(systemTime: Long, nodeCount: Int) => {
...
class nodeActor extends Actor {
  ...
  def receive = {
    ...
    case initNodes(nid: String, nodes: Array[Array[Array[ActorRef]]], rumorCondition: Int, cubeSize: Int, masterFromMain: ActorRef, sum: Double, weight: Double, topoType: String) => {
      nodeId = nid
      nodeArray = nodes
  ...
  if (args.length == 0 || args.length != 3) {
    ...
    if (topology == full || topology == line) {
      ...
      for {
        ...
        sumAssign += 1
        nodes(i)(0)(0) ! initNodes(nodeId, nodes, rumorLimit, numNodes, master, sumAssign, 1, topology)
      }

      //initiate Gossip
      if (algorithm == gossip) {
        nodes(0)(0)(0) ! SendGossip
        master ! initBuildTime(System.currentTimeMillis, numNodes)
      }
    ...
    if (topology == threeD || topology == imp3D) {
      ...
      for {
        ...
        sumAssign += 1
        nodes(i)(j)(k) ! initNodes(nodeId, nodes, rumorLimit, size, master, sumAssign, 1, topology)
      }
      ...
      if (algorithm == gossip) {
        ...
        master ! initBuildTime(System.currentTimeMillis, numberNodes)
      }
      if (algorithm == pushSum) {
